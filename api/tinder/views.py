...
from tinder.serializers import ActivityLikedSerializer, BaseActivitySerializer
...
class CreateActivityAPIView(APIView):
    def post(self, request: Request, *args, **kwargs):
        data = request.data
        activity, errors = BaseActivitySerializer.save_data(data)

        return CustomJsonResponse(
            data=BaseActivitySerializer(activity).data,
            errors=errors,
            success_code=status.HTTP_201_CREATED,
            error_code=status.HTTP_400_BAD_REQUEST,
        )
